...
如果把耳机简单看成一个电声转换器，会忽略大量真正决定其性能的因素：
...
因此，耳机的频率响应可以与 DAC 的频率响应形成非常鲜明的对比。优秀 DAC 的频响差异可能只有很小的分贝量级，而不同耳机之间可以轻易出现十几甚至二十多 dB 的差异。换句话说，在现代 HiFi 系统中，真正对最终声音产生巨大影响的，通常不是 DAC 是否从 -120 dB 变成 -125 dB，而是耳机本身如何把电信号转换成声压。
...
耳机的测量项目很多，但并不是所有指标都具有同等重要性。对于实际听感而言，频率响应通常是最重要的指标，而阻抗、灵敏度、THD、谐振和时间响应则分别从不同角度描述耳机的工作状态。
...
其中最重要的仍然是**频率响应**。原因很简单：人耳对频谱变化非常敏感，而耳机的频响偏差往往远大于现代电子设备的频响偏差。
...
入耳式耳机（IEM）的高频测量尤其容易出现差异。

很多不同测量机构测试同一副 IEM 时，会发现：
...
这意味着在 10kHz 左右，耳道已经不再能够被简单看成一个“无限小的点”。
...
因此，仅仅使用一个标准耦合器已经不足以完整描述头戴式耳机。

这就需要：

**HATS（Head And Torso Simulator，头部和躯干模拟器）。**
...
HATS 的意义在于把耳机放置在一个具有标准几何尺寸的“人”上。

这样，测量结果不仅包含耳机本身，还可以包含头部和耳廓对声波传播产生的影响。
...
即使使用人工耳或 HATS，不同实验室的结果仍然可能存在差异。
...
只有这些条件大致一致，频响曲线之间的比较才具有较强意义。

---

##### 从Free Field 到 Diffuse Field 再到 Harman Target & BK5128
...
其中一个思路是 **Free Field（自由场）**。

它假设声音主要来自正前方，测量系统希望模拟人在自由声场中面对一个声源时的听觉条件。

另一种重要思路是 **Diffuse Field（扩散场）**。

扩散场假设声音来自多个方向，耳朵因此会形成不同的频率响应。
...
因此出现了以大量受试者主观评价为基础的 **Harman Target**。
...
##### 夹具测量的局限与personal HRTF：统计学意义和个人极致体验的矛盾
...
这也是为什么“某条曲线在夹具上最接近 Harman”与“某个人听起来最好”并不是完全等价的命题。
...
虽然阻抗只有 32Ω，但如果效率较低，它仍然可能需要较大的输出功率。

因此：

> **阻抗决定负载特性，灵敏度决定需要多少输入才能得到目标声压。**
...
这也解释了为什么一个“标称 32Ω”的耳机，在高频并不一定始终保持 32Ω。

最终可以把阻抗理解成：

> **耳机整个电—机械—声学系统对外呈现出的等效负载。**
...
> **高阻耳机通常更需要电压，低阻低灵敏度耳机则可能更需要电流和功率。**
...
但其中一副可能主要产生二次谐波，另一副主要产生高阶谐波。

它们的失真结构并不相同。

因此，如果要深入分析失真，不能只看一个 THD 数字，还需要看：
...
这也是为什么耳机的时间响应并不是单纯由“振膜轻不轻”决定的。

一个非常轻的振膜，如果配合高 Q 的腔体，同样可能出现明显拖尾。
...
时，不同位置产生的声波开始出现明显相位差，不同区域的运动状态不同。
