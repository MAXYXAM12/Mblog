...
Jason Stoddard 在 Schiit 的 Gain 章节中给出的核心判断非常直接：系统应该有足够的增益，使功放在需要时能够达到最大输出，但过高增益会带来噪声和音量控制范围缩小的问题。其定量例子还指出，电平下降 3 dB 时，电压约为满幅的 71%，功率约为满幅的一半；下降 6 dB 时，电压约为 50%，功率约为四分之一。citeturn3search0
...
Roon 的 Volume Leveling 使用 ReplayGain/R128 分析统一不同曲目的感知响度，并将响度分析与峰值分析分开处理。Apple Music 的 Sound Check 同样会调整不同歌曲之间的播放响度，但具体归一化目标不应在这里被当成固定常数。citeturn6search0turn6search2turn6search3
...
Schiit 在 Gain 章节中特别提到，relay ladder 与传统电位器的旋钮位置并不能直接比较；不同衰减网络在相同听音电平下可能处于完全不同的旋钮位置。citeturn3search0
...
> **注意：LUFS 不能直接换算成 dBFS 峰值。计算器因此把 LUFS 与 True Peak 分开显示，不把二者强行相加。**

```html
<div id="amp-calculator">
  <style>
    #amp-calculator {
      max-width: 900px;
      margin: 1.5em auto;
    ...
    #amp-calculator label {
      display: block;
      font-weight: 600;
      margin-bottom: 4px;
    }
    #amp-calculator input,
    #amp-calculator select {
      width: 100%;
      box-sizing: border-box;
      padding: 7px;
    }
    #amp-calculator .result {
      margin-top: 16px;
      padding: 12px;
  ...
  <div class="grid">
    <div>
      <label>音乐平均 LUFS</label>
      <input id="lufs" type="number" value="-14" step="0.1">
    </div>

    <div>
      <label>True Peak / dBTP</label>
      <input id="peak" type="number" value="-1" step="0.1">
    </div>

    <div>
      <label>Track Gain / dB</label>
      <input id="track" type="number" value="0" step="0.1">
    </div>

    <div>
      <label>数字音量 / dB</label>
      <input id="digital" type="number" value="-20" step="0.1">
    </div>

    <div>
      <label>DAC 满幅输出 / Vrms</label>
      <input id="dac" type="number" value="2" step="0.01">
    </div>

    <div>
      <label>耳放增益 / dB</label>
      <input id="gain" type="number" value="6" step="0.1">
    </div>

    <div>
      <label>耳机阻抗 / Ω</label>
      <input id="imp" type="number" value="32" step="0.1">
    </div>

    <div>
      <label>耳机灵敏度</label>
      <input id="sens" type="number" value="105" step="0.1">
    </div>

    <div>
      <label>灵敏度单位</label>
      <select id="sensUnit">
        <option value="V">dB/V</option>
        <option value="mW">dB/mW</option>
      </select>
    </div>

    <div>
      <label>目标 SPL / dB</label>
      <input id="spl" type="number" value="90" step="0.1">
    </div>

    <div>
      <label>耳放输出阻抗 / Ω</label>
      <input id="rout" type="number" value="0.1" step="0.01">
    </div>
  </div>

  <div class="result" id="amp-result"></div>
  ...
  <script>
    ...
    function calcAmp() {
      ...
      const peak = n("peak");
      const track = n("track");
      const digital = n("digital");
      const dac = n("dac");
      const gainDb = n("gain");
      const imp = Math.max(n("imp"), 0.001);
      const sens = n("sens");
      const spl = n("spl");
      const rout = Math.max(n("rout"), 0);
      ...
      if (document.getElementById("sensUnit").value === "V") {
      ...
      const iAmpRequired =
        vAmpRequired / (imp + rout);

      const pAmpRequired =
        vAmpRequired * iAmpRequired;

      const clipping =
        vAmp >= vAmpRequired
          ? "当前耳放输出能力足以达到目标 SPL"
          : "当前输入电平/增益不足以达到目标 SPL";

      document.getElementById("amp-result").innerHTML = `
        <strong>计算结果</strong><br><br>
        音乐平均响度：${lufs.toFixed(1)} LUFS<br>
        True Peak：${peak.toFixed(1)} dBTP<br>
        Track Gain：${track.toFixed(1)} dB<br><br>

        DAC 当前输出：${vDac.toFixed(4)} Vrms<br>
        耳放当前理论输出：${vAmp.toFixed(4)} Vrms<br><br>

        耳机端所需电压：${vHp.toFixed(4)} Vrms<br>
        耳机端所需电流：${(iHp * 1000).toFixed(3)} mArms<br>
        耳机端所需功率：${(pHp * 1000).toFixed(3)} mW<br><br>

        考虑输出阻抗后耳放所需电压：
        ${vAmpRequired.toFixed(4)} Vrms<br>

        考虑输出阻抗后耳放所需电流：
        ${(iAmpRequired * 1000).toFixed(3)} mArms<br><br>
    ...
    document
      .querySelectorAll("#amp-calculator input, #amp-calculator select")
      .forEach(el => el.addEventListener("input", calcAmp));
...
Schiit 在 2024 年的 Feedback 章节中把问题讲得很直接：放大器中的有源器件本身并不理想，因此负反馈是用开环增益换取更高线性度的一种工程手段。其列出的典型收益包括更高带宽、更低失真、更低噪声和更低输出阻抗。citeturn3search2
...
Schiit 的技术文章还特别区分了 voltage feedback 与 current feedback。其核心观点不是“一个好一个坏”，而是两种结构各有工程取舍。citeturn2search0
...
Schiit 将自身大量放大器归入 current feedback 路线，但这并不意味着电流反馈天然“更有声音”或“更高级”。citeturn2search0
...
> **Class A 的核心不是“声音高级”，而是输出器件在规定工作范围内不被截止。** citeturn4search0
...
Schiit 给出的极端示例是：100 W / 4 Ω 的纯单端 A 类理论上需要非常夸张的静态功耗，因此大功率 A 类必须付出巨大的散热代价。citeturn4search0
...
Schiit 也将 White Cathode/E­mitter Follower 和图腾柱结构作为典型的推挽 A 类讨论对象。citeturn4search0
...
Schiit 对此的判断非常明确：大型“Class A”功放很多实际上属于 complementary Class A / high-bias AB 的灰色区域。citeturn4search0
...
Schiit 在 Balanced 章节中特别强调：看到 XLR 接口并不能证明内部一定是全差分，也不能证明一定获得了“平衡”的全部收益。真正需要看的是信号从输入到输出的拓扑。citeturn4file11turn4file4
...
那么“XLR”本身并不会神奇地创造差分放大优势。Schiit 对这种 Balanced Converting 拓扑也进行了专门说明。citeturn4file4
...
Schiit 在 Discrete 章节中强调，分立设计最大的优势之一就是**设计自由度**：可以自由选择低环路增益、电流反馈、差分、Class A、Class S、新型器件以及特殊拓扑。citeturn3search1
...
| **相位裕度** | 反馈环路距离不稳定状态的余量 | 环路补偿、负载 | 决定稳定性与振铃倾向 |
...
增益应该以**刚好能够在需要时达到最大无失真输出**为原则，而不是越高越好。过高增益会使音量控制长期处于大衰减区域，增加噪声、调节范围和声道匹配方面的压力；真正合理的系统应让 DAC 输出、数字/模拟衰减、耳放增益、耳机灵敏度和目标 SPL 共同匹配。Schiit 对增益的总结也可以概括为：**增益不足会导致系统无法达到最大输出，增益过高则容易带来噪声和较差的音量控制范围。** citeturn3search0
