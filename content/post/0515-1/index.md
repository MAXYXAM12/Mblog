...
**HQPlayer** 简单来说是一款强大的**软件 DSP（数字信号处理器）**，提供丰富的**升频/滤波/抖动/调制/卷积**选择，能有针对性地提升解码素质&微调音色
...
`Fixed Volume` 推荐打勾，相当于 -3dB gain ，提供一定 headroom 动态余量，防止 clipping 削波以及一些渣录混音源在升频后可能发生的爆音；如果不想要这点数字音量衰减导致的动态压缩和信息量减少，请设为 0dB ；如果你的前端系统只有数字音量衰减那可以考虑使用 HQPlayer 界面的旋钮作为音量控制
...
HQPlayer 的多项功能是直接与现代 DAC 的工作方式有关的，更深入的内容请见下一篇文章，这里只浅谈一下**升频**：

很多萌新对升频的第一印象来自网易云/ QQ 音乐的“臻品母带“（ AI 升频的 24bit 192khz ），在烧友口口相传和诸多科普之后，达成了“升频无用甚至有害“的共识（这两流媒体确实如此，大多为频谱一刀切的“假无损“再根据 AI 算法乱加超高频）。

需要明确的是，此类商业噱头性质的 AI 升频与 HQPlayer 毫不相关，升频本身也不应得到如此评价————事实上大部分音频设备中都存在升频！

升频更专业的名称是过采样 Oversampling ，简称 OS ；事实上只采用 NOS （非过采样）的“解码器“反而是少数。笔者在此不展开介绍，只提出一个问题供读者思考：我们接触到的录音文件里，24 bit 96khz 几乎已经是最高规格的了，为什么 HiFi “解码器” 往往宣传自己支持 384/768khz PCM 和 DSD256/512 ？
