...
然而真正科学 HiFi 的**门槛**很高——既要学习信号系统、模电数电、电声工程、心理声学，又要仪器测量、实践验证，若非相关专业从事者，需要投入大量时间精力。
...
当信号超出芯片的满量程上限，超出部分会被硬切，**削波**失真已在转换瞬间产生。

因此必须在 DAC 之前的**数字域进行衰减**，把波形峰值压回安全范围，DAC 才能正确还原原始信号。后级模拟衰减位于 DAC 之后，此时削波失真已经生成，它只能把已经失真的声音整体调小，如同把拍糊的照片缩小——失真不会消失，只是变轻了，无法挽回。
...
现代运放或者高性能耳放的**开环增益通常非常高**，但这种增益并不是稳定不变的常数。
...
工程师真正希望得到的却是一个**稳定、可预测**的系统，于是就有了反馈。
...
这也是后面讨论**带宽、GBW、相位裕度和稳定性**时必须牢牢记住的一条主线：
...
因此现代放大器往往不会直接把一个“低失真的高增益级”作为最终输出，而是使用**高开环增益+反馈网络**把最终性能重新定义出来。
...
也就是说**10 Ω 的输出阻抗本身就可能让这副耳机的频响发生约 1.5 dB 的变化。**
...
因此**耳放的输出阻抗只是整个阻尼系统中的一个环节。**

尤其对于**平板、静电**等结构，不能直接套用传统动圈领域的“阻尼系数”概念。
...
它还必须保证**没有音乐信号的时候，耳机端不要存在明显的直流电压。**
...
中间没有输出耦合电容。这种结构称为**Direct Coupling，直流耦合。**
...
也就是**只检测输出中的极低频 / DC 分量，然后利用另一个反馈环路把它抵消。**
...
| 方案 | 主要优势 | 主要代价 |
|---|---|---|
| 直流耦合 | 低频响应好、输出阻抗低、信号路径简单 | 必须严格控制 DC Offset |
| 电容耦合 | 天然隔离 DC，电路保护简单 | 需要大容量电容，形成高通 |
| 直流伺服 | 可以兼顾直流耦合和低 Offset | 电路更复杂，需要额外稳定性设计 |
...
然而真实放大器内部存在多个晶体管级、寄生电容、补偿电容以及不同的信号传播延迟。于是当频率升高时，反馈信号不再与输入信号保持简单的比例关系，而会产生越来越明显的**相位延迟**。
...
为了描述距离振荡条件还有多远，工程上使用**Phase Margin，PM，相位裕度。**
...
除了相位裕度，另一个常见指标是**Gain Margin，GM，增益裕度。**
...
这就是**Frequency Compensation，频率补偿。**
...
单一正弦波计算出的“刚好够用”的压摆率，并不能代表真实音乐下的最佳设计。
...
意味着小信号情况下，频率到达约 100 kHz 时开始进入规定的衰减范围。

而 Slew Rate 描述：

> **一个大幅度信号的电压变化速度是否超过内部最大速度。**
...
> **“这台耳放带宽很高”不能证明它在大信号下拥有足够的压摆率。**

反过来也一样：

> **Slew Rate 很高，也不能证明小信号环路一定稳定。**
...
> **如果单一 THD 不能说明一切，那么现有耳放测量能否完整反映瞬态能力？**

答案是：

> **不能靠单一指标完成，但多种测试可以从不同角度共同观察。**

例如：

| 测试 | 主要观察 |
|---|---|
| 1 kHz THD+N | 基本线性度与噪声 |
| 高频 THD | 高频非线性 |
| 高输出电平 THD | 大信号非线性 |
| IMD | 多频同时作用时的非线性 |
| Multitone | 更接近复杂频谱 |
| 方波响应 | 带宽、相位、过冲、振铃 |
| 大信号方波 | Slew Rate 与大信号能力 |
| 不同负载 | 输出级负载适应能力 |
| 容性负载 | 环路稳定性 |
| 热稳定性 | 长时间工作状态 |
...
当这些频率同时进入非线性放大器时，它们之间会相互调制。

这就是 IMD。

因此：

> **多音测量比单音 THD 更接近复杂音乐的频谱条件，但它仍然不能完全等价于真实音乐。**
...
其重点不再是使用高阻差分输入节点直接进行传统电压误差放大，而是通过一个低阻抗反馈节点感知电流。
...
|---|---|---|
| 主要反馈变量 | 电压 | 电流 |
| 常见输入结构 | 差分输入级 | 缓冲器/低阻反馈节点 |
| 增益与带宽 | 通常具有明确 GBW 关系 | 带宽更多取决于反馈网络 |
| 压摆率 | 通常受内部电流与补偿影响 | 可实现非常高的压摆率 |
| 反馈电阻敏感性 | 通常相对较低 | 通常更高 |
| 高频设计难度 | 相对成熟 | 对反馈网络和寄生参数更敏感 |
| 典型应用 | 通用精密音频运放 | 高速、宽带放大器 |
...
这几个词在 HiFi 市场中经常被直接与“音质”联系起来，但从电子学定义来看，它们首先描述的是：

> **输出器件的导通方式以及功率转换方式。**

因此：

> **Class A 不是“高级”的同义词，Class D 也不是“数字”的同义词。**
...
所以 A 类只是消除了某一类典型的截止问题。

---

#### A 类最大的代价：效率

假设一个单端 A 类输出级的静态电流始终存在。
...
与单管持续承受整个输出相比，推挽结构可以更有效地利用电源和输出器件。

但它仍然保持足够的静态偏置，使输出器件在规定的 A 类工作范围内不会进入截止。

因此：

> **推挽 A 类仍然属于 A 类，但它比简单的单端 A 类更容易实现较大的输出能力。**

---

#### AB 类

AB 类是实际音频放大器中非常重要的折中方案。
...
> **用一部分静态电流换取更好的交越区域线性，同时比纯 A 类具有更高效率。**
...
范围内处于 A 类，但达到更高输出功率后进入 AB。

从工程角度，这完全是合理设计。

因为耳机正常听音时可能只需要很小的功率，而厂商希望在这个最常用的工作范围内避免明显的交越问题，同时又希望在需要更大功率时保持合理效率。

因此判断一个“Class A 耳放”到底是什么，应该看：
...
**Crossover Distortion，交越失真。**

因此：

> **AB 类本质上就是为了解决 B 类的交越问题，同时保留 B 类较高效率的一种折中方案。**
...
MOSFET 不再像传统 A/AB 类那样长期处于线性放大状态，而主要在：
...
> **如何让一个高速开关系统在尽量低的损耗下还原出低失真的模拟信号。**
...
> **D 类的难点不是“音频放大不了”，而是如何控制高速开关系统产生的非理想效应。**
...
| 类型 | 主要优势 | 主要代价 |
|---|---|---|
| A 类 | 输出器件持续导通，低电平区域容易获得连续工作 | 静态功耗大、发热高、效率低 |
| 推挽 A 类 | 相比单端 A 类更容易实现较高输出能力 | 仍然存在较高静态功耗 |
| AB 类 | 线性与效率之间的折中 | 需要处理偏置与交越失真 |
| B 类 | 效率高 | 交越失真明显 |
| D 类 | 极高效率、低发热、高功率密度 | 高频开关、EMI、输出滤波与负载耦合复杂 |
...
> **在目标输出功率、负载、效率、散热、噪声、线性度和体积约束下，哪一种拓扑更适合这个任务？**

对于耳放来说尤其如此。

一个设计优秀的 AB 类完全可以获得极低失真；一个设计优秀的 D 类同样可以获得非常好的线性和噪声性能；而一个设计糟糕的 A 类，也完全可能拥有很高的失真、噪声或者输出阻抗。

因此：

> **Class A、AB、D 首先是工程分类，而不是音质等级。**

这与前面负反馈、输出阻抗以及增益的讨论最终都会汇合到同一个结论：
