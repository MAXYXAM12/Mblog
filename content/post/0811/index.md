...
耳放并不是"把声音变大"的黑盒子,而是 DAC 输出与耳机之间负责**电压、电流、增益、阻抗匹配和线性度**的模拟信号处理环节。
...
DAC 的模拟输出通常面向的是**高阻抗的线路输入**,而不是低阻抗的耳机。
...
DAC 输出级的典型设计目标是在高阻抗负载下提供低失真、低噪声的电压输出,其输出级通常采用射极跟随器或运放缓冲,输出电流能力一般在几十毫安以内。当负载阻抗从 $10k\Omega$ 降到 $32\Omega$,输出电流需求增加了两个数量级以上,这已经超出了绝大多数 DAC 输出级的安全工作区。

此外,DAC 输出级直接驱动低阻负载还会面临:

- **输出阻抗不匹配**:DAC 输出阻抗通常在几十到几百欧姆,与低阻耳机之间会产生显著的电压分配损失;
- **非线性加载效应**:低阻抗负载会改变 DAC 输出级的工作点,引入额外失真;
- **热保护与限流**:许多 DAC 芯片内部有限流保护,过载时输出会被钳位。
...
- **THD(总谐波失真)**:非线性产生谐波;当放大器的传递函数不是严格的线性时,输出中会出现输入频率的整数倍分量。THD 通常以基波幅度的百分比或分贝表示。
- **IMD(互调失真)**:多个频率相互调制产生额外频率成分;对于音乐这种多频信号,IMD 往往比 THD 更能反映实际听感中的非线性问题。
- **频率响应误差**:不同频率的增益不同;理想耳放在 20Hz-20kHz 范围内应保持平坦。
- **噪声**:没有输入信号时仍然存在输出;包括热噪声、散粒噪声、1/f 噪声等。
- **输出阻抗**:负载变化会影响实际输出电压。
...
例如,如果 DAC 满幅输出为 2 Vrms,而耳放输入灵敏度恰好为 2 Vrms(增益 6 dB,最大输出 4 Vrms),那么 DAC 满幅时耳放刚好达到最大输出--这是一个理想的增益匹配。但如果耳放灵敏度为 0.5 Vrms(增益 20 dB,最大输出 5 Vrms),那么 DAC 输出只需 1/4 就能达到耳放最大输出,音量控制器将长期处于大衰减区域。
...
所以"20 dB 增益"并不是"多一点增益",而是**电压放大 10 倍**。
...
削波发生时,输出波形的峰值被"削平",产生大量高次谐波失真。对于音乐信号,削波往往不是持续的,而是出现在瞬态峰值处--鼓点、钢琴强音、铜管齐奏等。这些瞬态削波可能不会立刻被听觉系统识别为"失真",但会造成声音发硬、疲劳感增加、声场压缩等问题。
...
它描述的是**耳朵实际接收到的声压**,而不是 DAC 或耳放内部的电压。
...
Fletcher-Munson 等响曲线(后经 ISO 226 修订)描述了这一现象:
...
在后面的计算中,可以把听音声压暂时分成三个**仅用于系统设计的示例档位**:
...
首先,耳放需要满足的不是某个固定的"平均声压",而是**音乐在平均电平之上的峰值余量**。

#### LUFS

LUFS(Loudness Units Full Scale)是 ITU-R BS.1770 标准定义的感知响度单位。它与简单的 dBFS 有本质区别:

- **dBFS** 衡量的是信号的**瞬时或峰值电平**,不考虑人耳的频率感知特性;
- **LUFS** 衡量的是经过**K-加权滤波**(模拟人耳对不同频率的灵敏度差异)和**时间积分**后的**感知响度**。

K-加权滤波器的频率响应大致反映了人耳的等响特性:在 1-5 kHz 中频区域有适度提升,在低频和极高频有衰减。因此 LUFS 比 dBFS 更能反映人耳实际感受到的"响度"。

LUFS 分为两种:

- **Momentary LUFS**:400 ms 时间窗口内的瞬时响度,反映音乐的瞬态变化;
- **Integrated LUFS**:整首曲目从头到尾的平均感知响度,是最常用的响度归一化参考值。

例如,Spotify 的响度归一化目标约为 -14 LUFS(Integrated),Apple Music 的 Sound Check 目标约为 -16 LUFS。这意味着如果一首曲目的 Integrated LUFS 为 -10 LUFS,播放器会将其衰减约 4-6 dB 以匹配目标。
...
dBTP(dB True Peak)衡量的是信号的**真实峰值电平**,而非采样点处的离散峰值。其区别在于:

- 普通 dBFS peak 只看采样点的最大值;
- dBTP 通过 4 倍过采样插值,检测采样点**之间**可能存在的更高瞬时峰值。

这在数字音频中很重要:当两个相邻采样点的值都接近满幅时,它们之间的模拟波形可能超过 0 dBFS,导致 DAC 输出削波。EBU R128 标准要求广播音频的 True Peak 不超过 -1 dBTP,流媒体平台通常要求 -1 dBTP 或 -2 dBTP。

对于耳放系统设计,dBTP 的意义在于:它决定了数字信号的**实际峰值**,进而影响对耳放电压余量的要求。

#### LUFS 与 dBTP 的关系

一首典型曲目的 Integrated LUFS 与 dBTP 之差反映了其**峰值余量(Peak-to-Loudness Ratio, PLR)**:
...
| 曲目类型 | Integrated LUFS | dBTP | PLR |
|---|---|---|---|
| 古典交响(大动态) | -22 LUFS | -2 dBTP | 20 dB |
| 爵士现场 | -18 LUFS | -3 dBTP | 15 dB |
| 流行/摇滚 | -12 LUFS | -2 dBTP | 10 dB |
| 电子/舞曲(强压缩) | -9 LUFS | -1 dBTP | 8 dB |

PLR 越大,说明音乐的动态范围越大,耳放需要保留的峰值余量越多。

#### Track Gain 与 EQ Headroom

**Track Gain**(也称 ReplayGain Track Gain)是 ReplayGain 系统为每首曲目计算的增益调整值,目的是让不同曲目在播放时达到相同的感知响度。其计算方式是:

$$
Track\ Gain_{dB} = Target\ Loudness_{LUFS} - Track\ Loudness_{LUFS}
$$

例如,如果目标响度为 -14 LUFS,某曲目的 Integrated LUFS 为 -20 LUFS,则 Track Gain 为 +6 dB。播放器会将该曲目在数字域提升 6 dB。

这对耳放系统的影响是:**经过 Track Gain 调整后,信号的峰值也会相应变化**。如果原始曲目的 dBTP 为 -6 dBTP,经过 +6 dB Track Gain 后,峰值变为 0 dBTP--已经到达数字满幅。因此:

$$
Peak_{after\ gain} = dBTP_{original} + Track\ Gain
$$

**EQ Headroom** 是在使用均衡器时需要预留的额外余量。如果 EQ 在某个频段提升了 +6 dB,那么该频段的峰值也会增加 6 dB。为了防止数字域削波,通常建议在 EQ 之前先衰减整体信号,衰减量至少等于 EQ 的最大提升量。

对于耳放系统设计,这意味着:

$$
V_{required} = V_{SPL} \times 10^{PLR/20} \times 10^{EQ_{headroom}/20}
$$

其中 $V_{SPL}$ 是达到目标平均声压所需的电压,$PLR$ 是音乐的峰值余量,$EQ_{headroom}$ 是 EQ 可能引入的最大提升量。

因此,同样的平均听音声压,大动态古典音乐可能比压缩过的流行音乐多需要 10-15 dB 的峰值余量--这对耳放的电压摆幅和压摆率都提出了更高要求。
...
前者要求耳放保留更多**瞬态电压、电流和功率余量**;后者虽然平均声压可能更高,但峰值余量要求反而可能较低。

典型的动态范围数据:

| 音乐类型 | 平均电平(参考) | 峰值余量需求 |
|---|---|---|
| 古典交响(大动态) | -20 ~ -25 LUFS | 15-20 dB |
| 爵士现场 | -16 ~ -20 LUFS | 10-15 dB |
| 流行/摇滚(压缩后) | -10 ~ -14 LUFS | 6-10 dB |
| 电子/舞曲(强压缩) | -8 ~ -12 LUFS | 3-6 dB |
...
如果灵敏度以 dB/mW 表示,需要先转换为 dB/V:

$$
Sensitivity_{dB/V}=Sensitivity_{dB/mW}+10\log_{10}\left(\frac{1000}{Z_{HP}}\right)
$$
...
| 耳机特征 | 主要压力 | 典型问题 |
|---|---|---|
| 高阻 + 高敏 | 电压需求不高 | 更关注增益、噪声和余量 |
| 高阻 + 低敏 | 电压 | 输出摆幅不足 |
| 低阻 + 高敏 | 电流需求不一定大 | 噪声、输出阻抗、细调范围 |
| 低阻 + 低敏 | 电压 + 电流 | 真正考验输出级能力 |
...
| 变量 | 改变什么 | 对耳放的影响 |
|---|---|---|
| 音乐动态 | 峰值与平均值的距离 | 决定峰值余量 |
| 耳机负载 | 电压、电流需求 | 决定输出能力 |
| 听音电平 | 整体目标声压 | 决定所需绝对输出 |
