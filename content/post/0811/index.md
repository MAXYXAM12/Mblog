...
但即使已经拥有足够的电压、电流和功率，仍然存在另一个问题：

> **耳放会不会因为自身的输出阻抗，改变耳机原本的工作状态？**

这是理解“推得响”和“推得好”区别时非常重要的一步。
...
> **10 Ω 的输出阻抗本身就可能让这副耳机的频响发生约 1.5 dB 的变化。**
...
因此现代固态耳放普遍追求低输出阻抗，并不只是因为“低阻更有力”，而是因为：

> **低输出阻抗能够减少耳放与耳机阻抗曲线之间的耦合。**

---

#### 输出阻抗与灵敏度

前面的耳放需求计算中，我们使用的是耳机厂商标出的灵敏度：

$$
Sensitivity
$$

但严格来说，耳机的实际声压响应还受到输出阻抗影响。
...
> **厂商标注的“某某 dB/V”并不是脱离驱动设备永远固定不变的声压值。**

在正常低输出阻抗耳放上，这种差异往往很小，所以工程计算时仍然可以使用厂商标注值。

但当输出阻抗达到十几欧姆、几十欧姆时，就不能再忽略这一点。
...
> **低输出阻抗的真正价值，是让耳放尽可能接近理想电压源，而不是制造所谓“更强的声音”。**

---

### 16. 非纯阻负载：反电动势与容性

前面的计算为了方便，通常把耳机近似成一个纯电阻：
...
Z(f)
=
R_e+j\omega L_e+Z_{mechanical}(f)
$$
...
> **耳放和耳机之间不是单向关系，而是一个双向耦合系统。**

耳放施加电压会改变耳机的运动，而耳机的运动又会改变耳放端看到的电气状态。
...
如果耳放具有较低的输出阻抗，那么这个反电动势会更容易在低阻抗输出级中形成电流通路。
...
因此“低 Rout = 更快瞬态”不能作为单独成立的因果关系。

真正应该说的是：

> **低输出阻抗可以减少电气接口本身对换能器工作状态的影响。**
...
> **能够在高电压摆幅下，稳定驱动一个高度容性的负载。**

这也是为什么在静电系统中，“功率”这个指标的解释力远低于在低阻动圈耳机中的解释力。
...
> **只检测输出中的极低频 / DC 分量，然后利用另一个反馈环路把它抵消。**
...
| DC Servo | 可以兼顾直流耦合和低 Offset | 电路更复杂，需要额外稳定性设计 |

因此：

> **输出是否采用耦合电容，不应该被简单理解成“高级”和“低级”的区别，而是 DC 管理方式不同。**
