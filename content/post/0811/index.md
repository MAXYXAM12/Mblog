...
如果耳机阻抗 $ Z_{HP}(f) $ 随频率变化，那么耳放输出阻抗会改变不同频率下实际获得的电压：

$$ V_{HP}(f) = V_{amp}(f) \frac{Z_{HP}(f)} {Z_{HP}(f)+R_{out}} $$

因此 $ Sensitivity(f) $ 也会随 $R_{out}$ 变化。

换句话说，**厂商标注的“某某 dB/V”并不是脱离驱动设备永远固定不变的声压值。**
...
输出阻抗越低，阻尼系数通常越高，但“阻尼系数越高越好”其实并不是一个完整的工程结论。
